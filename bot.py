...
# ==================== RENDER WEB SERVICE UCHUN HEALTH-CHECK SERVER ====================
# Render "Web Service" turi tashqi portni tinglashni talab qiladi.
# Bot o'zi Telegram bilan polling orqali ishlaydi, shu server esa
# faqat Render'ga "servis tirik" ekanini bildirish uchun kerak.
...
def upload_to_imgbb_sync(image_bytes):
    ...
    payload = {
        "key": IMGBB_API_KEY,
        "image": base64.b64encode(bytes(image_bytes)).decode("utf-8"),
    }
    try:
        response = requests.post(url, data=payload, headers=headers, timeout=30).json()
...
if __name__ == "__main__":
    # Health-check serverni alohida (fon) threadda ishga tushiramiz,
    # asosiy thread esa Telegram polling bilan band bo'ladi.
